...
# cluster 값이 0, 1, 2 인 경우마다 별도의 Index로 추출
    ...
    plt.scatter(x=pca_center[i,0], y=pca_center[i,1], s=300, color='white', alpha=0.9, edgecolor='k', marker=markers[i])
...
df.to_csv('result_kmeans_논문_4_이상값제거_utf.csv', encoding = 'utf-8', index=False)


#spss용 데이터
df_spss = df[['area','capacity','realty_price','insolation','generation_quantity','economic_feasibility','result']]
df_spss.to_csv('SPSS_kmeans_논문_4_이상값제거_utf.csv', encoding = 'utf-8', index=False)
...
result_group_analysis.to_csv('after_analysis_논문_4군집_이상값제거.csv', encoding = 'cp949', index=False)

result_total_analysis = df.aggregate(['mean','std'])
result_total_analysis.to_csv('after_analysis_total_논문_4군집_이상값제거.csv', encoding = 'cp949', index=False)